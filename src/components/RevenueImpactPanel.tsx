...
export default function RevenueImpactPanel({ customers }: { customers: Customer[] }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-neutral-800 bg-neutral-900 p-5">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-neutral-800 text-green-400 ring-1 ring-inset ring-green-900">
        <ShieldCheck size={16} aria-hidden="true" />
      </span>
      <p className="text-sm leading-relaxed text-neutral-300">
        Acting on your {atRiskCustomers.length} at-risk customers could protect an estimated{" "}
        <span className="text-base font-semibold text-green-400">{formatCurrency(recoverable)}</span>
        <span className="text-neutral-400">/month</span> in recurring revenue.
      </p>
    </div>
  );
}
